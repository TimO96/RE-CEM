...
class AEADEN:
    def __init__(self, sess, model, mode, AE, batch_size, kappa, init_learning_rate,
        ...
        image_size, num_channels, num_classes = model.image_size, model.num_channels, model.num_labels
        ...
        self.target_lab = torch.zeros((batch_size, num_classes), dtype=torch.float32)
        self.const = torch.zeros((batch_size,), dtype=torch.float32, requires_grad=True)
    ...
    def loss():
        ...
        self.EN_dist = self.L2_dist + self.L1_dist * self.beta
        self.EN_dist_s = self.L2_dist_s + self.L1_dist_s * self.beta
    ...
    def fista():
        ...
        self.adv_img = self.assign_adv_img
        self.adv_img_s = self.assign_adv_img_s

    def reset_values():
        self.adv_img._zeros()
        self.adv_img_s._zeros()
        self.global_step._zeros()

    def setup(img_batch, label_batch, const):
